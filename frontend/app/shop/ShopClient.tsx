...
type Facets = { collections: string[]; categories: string[] };

export default function ShopClient({
  initialProducts, initialTotal, initialFacets,
}: {
  ...
  // ── Initialise from URL params (server already rendered the unfiltered first page) ──
  ...
  const [collections, setCollections] = useState<string[]>(
    Array.from(new Set([...DEFAULT_COLLECTIONS.filter(c => initialFacets.collections.includes(c)), ...initialFacets.collections]))
  );
  const [types, setTypes] = useState<string[]>(
    Array.from(new Set([...DEFAULT_TYPES.filter(t => initialFacets.categories.includes(t)), ...initialFacets.categories]))
  );

  // Skip the very first client fetch when there's no filter — server already provided that data
  const hasInitialFilter = !!(params.get('search') || params.get('collection') || params.get('category'));
  const [skipFirstFetch, setSkipFirstFetch] = useState(!hasInitialFilter);
