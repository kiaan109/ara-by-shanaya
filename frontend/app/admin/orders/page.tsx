    ...
    <div className="p-5 lg:p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] text-white/30 mt-1">{orders.length} total · ₹{totalRevenue.toLocaleString('en-IN')} revenue</p>
        </div>
        <button onClick={exportCSV}
          className="flex items-center gap-2 border border-white/[0.1] hover:border-[#C5A059]/50 text-white/50 hover:text-[#C5A059] px-4 py-2 text-[10px] tracking-[0.15em] uppercase transition-all rounded">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: 'Pending',   value: orders.filter(o => o.status === 'pending_payment').length, color: 'text-amber-400' },
          { label: 'Cancelled', value: orders.filter(o => o.status === 'cancelled').length, color: 'text-red-400' },
        ].map(s => (
          <div key={s.label} className="bg-[#1a1a1a] border border-white/[0.06] rounded-lg p-4 text-center">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="flex-1 bg-[#1a1a1a] border border-white/[0.08] rounded px-4 py-2.5 text-[12px] text-white placeholder:text-white/20 focus:outline-none focus:border-[#C5A059]/40" />
        <div className="flex gap-1.5 flex-wrap">
          {['all', ...STATUSES].map(s => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-3 py-1.5 rounded text-[9px] tracking-[0.12em] uppercase transition-colors ${
                filter === s ? 'bg-[#C5A059] text-black' : 'bg-[#1a1a1a] border border-white/[0.08] text-white/40 hover:border-[#C5A059]/30 hover:text-white/70'
              }`}>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-[11px] tracking-[0.2em] uppercase text-white/20">No orders found</div>
      ) : (
        <div className="space-y-1.5">
          {visible.map(order => {
            ...
            return (
              <div key={order.orderId} className="bg-[#1a1a1a] border border-white/[0.06] rounded-lg overflow-hidden">
                {/* Row */}
                <div className="px-5 py-3.5 flex flex-wrap items-center gap-4 cursor-pointer hover:bg-white/[0.02] transition-colors"
                  onClick={() => setExpanded(isOpen ? null : order.orderId)}>
                  <span className="font-mono text-[11px] text-white/50 w-44 flex-shrink-0">{order.orderId}</span>
                  <span className="text-[11px] text-white/30 w-20 flex-shrink-0">
                    {new Date(order.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                  </span>
                  <span className="text-[12px] flex-1 min-w-0 truncate">{order.name}</span>
                  <span className="text-[11px] text-white/30 hidden md:block flex-1 min-w-0 truncate">{order.email}</span>
                  ...
                  <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"
                    className={`transition-transform flex-shrink-0 text-white/30 ${isOpen ? 'rotate-180' : ''}`}>
                    <path d="M6 9l6 6 6-6"/>
                  </svg>
                </div>
                ...
                {/* Expanded */}
                {isOpen && (
                  <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} className="overflow-hidden border-t border-white/[0.06]">
                    <div className="px-5 py-5 grid md:grid-cols-2 gap-6">
                      {/* Items */}
                      <div>
                        <p className="text-[10px] tracking-[0.2em] uppercase text-white/30 mb-3">Items Ordered</p>
                        <div className="space-y-2.5">
                          {(order.items || []).map((item: any, i: number) => (
                            <div key={i} className="flex gap-3">
                              {item.image && <img src={item.image} alt="" className="w-10 h-12 object-contain bg-[#111] rounded flex-shrink-0" />}
                              <div>
                                <p className="text-[12px]">{item.name}</p>
                                <p className="text-[11px] text-white/30">{item.size ? `Size ${item.size} · ` : ''}Qty {item.quantity}</p>
                                <p className="text-[12px] text-[#C5A059]">₹{(item.price * item.quantity).toLocaleString('en-IN')}</p>
                              </div>
                            </div>
                          ))}
                        </div>
                        <div className="mt-3 pt-3 border-t border-white/[0.06] text-[12px] space-y-0.5 text-white/50">
                          <p>Total: <span className="text-[#C5A059] font-medium">₹{(order.total || 0).toLocaleString('en-IN')}</span></p>
                          {order.paymentId && <p className="text-[10px] text-white/20 mt-1">Razorpay: {order.paymentId}</p>}
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          <p className="text-[10px] tracking-[0.2em] uppercase text-white/30 mb-2">Customer</p>
                          <p className="text-[12px]">{order.name}</p>
                          <p className="text-[11px] text-white/40">{order.email}</p>
                          <p className="text-[11px] text-white/40">{order.phone}</p>
                          <p className="text-[11px] text-white/30 mt-1">{order.address}, {order.city}, {order.state} – {order.pincode}</p>
                        </div>
                        <div>
                          <p className="text-[10px] tracking-[0.2em] uppercase text-white/30 mb-2">Update Status</p>
                          <div className="space-y-2">
                            <select
                              value={curStatus}
                              onChange={e => setNewStatus(p => ({ ...p, [order.orderId]: e.target.value }))}
                              className="w-full bg-[#111] border border-white/[0.1] text-white px-3 py-2 text-[12px] focus:outline-none focus:border-[#C5A059]/40 rounded"
                            >
                            ...
                            <input
                              ...
                              className="w-full bg-[#111] border border-white/[0.1] text-white px-3 py-2 text-[12px] focus:outline-none focus:border-[#C5A059]/40 rounded placeholder:text-white/20"
                            />
                            <button
